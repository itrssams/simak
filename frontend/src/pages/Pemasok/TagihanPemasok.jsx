...
const STATUS_COLOR = {
    ...
    diterima: { bg: '#dbeafe', color: '#1e40af' },
    sebagian: { bg: '#fff7ed', color: '#c2410c' },
    lunas: { bg: '#dcfce7', color: '#166534' },
    batal: { bg: '#fee2e2', color: '#991b1b' },
};
const STATUS_LABEL = { draft: 'Draft', diterima: 'Diterima', sebagian: 'Dibayar Sebagian', lunas: 'Lunas', batal: 'Dibatalkan' };
...
const generateNomor = () => { const n = new Date(); return `TAG-${n.getFullYear()}${String(n.getMonth() + 1).padStart(2, '0')}${String(n.getDate()).padStart(2, '0')}-${String(n.getTime()).slice(-4)}`; };
const emptyItem = { deskripsi: '', kuantitas: 1, harga_satuan: '', subtotal: 0 };

export default function TagihanPemasok() {
    ...
    const [bayarForm, setBayarForm] = useState({ tanggal: new Date().toISOString().split('T')[0], jumlah: '', metode: 'transfer', keterangan: '', akun: '' });
    ...
    const fetchAll = async () => {
        ...
        try {
            const [t, p, a] = await Promise.all([
                api.get('/keuangan/tagihan/', { params: pageParams(page, pageSize) }),
                api.get('/keuangan/pemasok/'),
                api.get('/keuangan/akun/'),
            ]);
            setList(getResults(t.data));
            setTotal(getCount(t.data));
            setPemasokList(getResults(p.data));
    ...
    const openForm = () => {
        setForm({ nomor_tagihan: generateNomor(), nomor_ref_pemasok: '', tanggal: new Date().toISOString().split('T')[0], jatuh_tempo: '', pemasok: '', keterangan: '', status: 'draft', items: [{ ...emptyItem }] });
        setError('');
        setView('form');
    };
    ...
    const handleSave = async () => {
        setError('');
        if (!form.pemasok || !form.tanggal || !form.jatuh_tempo) { setError('Pemasok, tanggal, dan jatuh tempo wajib diisi.'); return; }
        const validItems = form.items.filter(i => i.deskripsi && Number(i.harga_satuan) > 0);
        if (validItems.length === 0) { setError('Minimal 1 item tagihan.'); return; }
        setSaving(true);
        try {
        ...
        } catch { setError('Gagal menyimpan tagihan. Pastikan nomor tagihan belum dipakai.'); }
        finally { setSaving(false); }
    };

    const openDetail = async (item) => {
        try {
            const res = await api.get(`/keuangan/tagihan/${item.id}/`);
            setSelected(res.data);
            setView('detail');
            setError('');
        } catch { setError('Gagal memuat detail tagihan.'); }
    };

    const handleTerima = async () => {
        try {
            await api.post(`/keuangan/tagihan/${selected.id}/terima/`);
            setSuccess('Tagihan berhasil diterima!');
            const res = await api.get(`/keuangan/tagihan/${selected.id}/`);
            setSelected(res.data);
            fetchAll();
            setTimeout(() => setSuccess(''), 3000);
        } catch (e) { setError(e.response?.data?.error || 'Gagal menerima tagihan.'); }
    };

    const handleBatal = async () => {
        if (!window.confirm('Batalkan tagihan ini?')) return;
        try {
            await api.post(`/keuangan/tagihan/${selected.id}/batal/`);
            setSuccess('Tagihan dibatalkan.');
            const res = await api.get(`/keuangan/tagihan/${selected.id}/`);
            setSelected(res.data);
            fetchAll();
            setTimeout(() => setSuccess(''), 3000);
    ...
    const filtered = list.filter(i => {
        ...
        const matchSearch = i.nomor_tagihan.toLowerCase().includes(search.toLowerCase()) || (i.pemasok_detail?.nama || '').toLowerCase().includes(search.toLowerCase());
        return matchStatus && matchSearch;
    });

    // Summary cards
    const totalBelumBayar = list.filter(i => ['diterima', 'sebagian'].includes(i.status)).reduce((a, b) => a + Number(b.sisa_tagihan || 0), 0);
    const totalDraft = list.filter(i => i.status === 'draft').length;
    const totalJatuhTempo = list.filter(i => ['diterima', 'sebagian'].includes(i.status) && new Date(i.jatuh_tempo) < new Date()).length;
    ...
    return (
        <div>
            ...
            <div style={S.header}>
                <div>
                    <h1 style={S.title}>Tagihan Pemasok</h1>
                    <p style={S.sub}>{list.length} tagihan tercatat</p>
                </div>
                <div style={{ display: 'flex', gap: '10px' }}>
                    {view !== 'list' && <button className="btn-secondary" onClick={() => { setView('list'); setError(''); }}>← Kembali</button>}
                    {view === 'list' && <button className="btn-primary" onClick={openForm}>+ Tagihan Baru</button>}
                </div>
            </div>
            ...
            {view === 'list' && (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '16px', marginBottom: '24px' }}>
                    <div style={S.summaryCard}>
                        <div style={{ fontSize: '12px', color: '#64748b', marginBottom: '6px', textTransform: 'uppercase', fontWeight: '600' }}>Belum Dilunasi</div>
                        <div style={{ fontSize: '22px', fontWeight: '700', color: '#dc2626' }}>Rp {formatRp(totalBelumBayar)}</div>
                    </div>
                    <div style={S.summaryCard}>
                        <div style={{ fontSize: '12px', color: '#64748b', marginBottom: '6px', textTransform: 'uppercase', fontWeight: '600' }}>Tagihan Draft</div>
                        <div style={{ fontSize: '22px', fontWeight: '700', color: '#475569' }}>{totalDraft} tagihan</div>
                    </div>
                    <div style={S.summaryCard}>
                        <div style={{ fontSize: '12px', color: '#64748b', marginBottom: '6px', textTransform: 'uppercase', fontWeight: '600' }}>Jatuh Tempo</div>
                        <div style={{ fontSize: '22px', fontWeight: '700', color: totalJatuhTempo > 0 ? '#dc2626' : '#166534' }}>{totalJatuhTempo} tagihan</div>
                    </div>
                </div>
            )}
            ...
            {view === 'list' && (
                <>
                    <div style={{ display: 'flex', gap: '12px', marginBottom: '16px' }}>
                        <input className="fi" style={{ width: '240px' }} placeholder="Cari nomor / pemasok..." value={search} onChange={e => setSearch(e.target.value)} />
                    ...
                    <div style={S.card}>
                        {loading ? <div style={S.empty}>Memuat...</div> : filtered.length === 0 ? <div style={S.empty}>Belum ada tagihan.</div> : (
                            <table className="dt">
                                <thead><tr>
                                    <th>No. Tagihan</th><th>Pemasok</th><th>Tanggal</th><th>Jatuh Tempo</th>
                                    <th style={{ textAlign: 'right' }}>Total</th><th style={{ textAlign: 'right' }}>Sisa</th><th>Status</th>
                                </tr></thead>
                                <tbody>
                                    {filtered.map(t => (
                                        <tr key={t.id} onClick={() => openDetail(t)}>
                                            <td style={{ fontWeight: '600', color: '#1a4731', fontFamily: 'monospace' }}>{t.nomor_tagihan}</td>
                                            <td>{t.pemasok_detail?.nama}</td>
                                            <td>{formatTgl(t.tanggal)}</td>
                                            <td style={{ color: new Date(t.jatuh_tempo) < new Date() && t.status !== 'lunas' ? '#dc2626' : '#334155', fontWeight: new Date(t.jatuh_tempo) < new Date() && t.status !== 'lunas' ? '600' : '400' }}>
                                                {formatTgl(t.jatuh_tempo)}
                                                {new Date(t.jatuh_tempo) < new Date() && !['lunas', 'batal'].includes(t.status) && <span style={{ marginLeft: '6px', fontSize: '11px', background: '#fee2e2', color: '#dc2626', padding: '2px 6px', borderRadius: '4px' }}>Lewat</span>}
                                            </td>
                                            <td style={{ textAlign: 'right', fontFamily: 'monospace' }}>Rp {formatRp(t.total_tagihan)}</td>
                                            <td style={{ textAlign: 'right', fontFamily: 'monospace', color: Number(t.sisa_tagihan) > 0 ? '#dc2626' : '#166534', fontWeight: '600' }}>Rp {formatRp(t.sisa_tagihan)}</td>
                                            <td><span style={{ display: 'inline-block', padding: '3px 10px', borderRadius: '6px', fontSize: '11px', fontWeight: '600', ...STATUS_COLOR[t.status] }}>{STATUS_LABEL[t.status]}</span></td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
            ...
            {view === 'form' && (
                <div style={S.card}>
                    <span style={S.sectionTitle}>Tagihan Baru</span>
                    {error && <div style={{ ...S.err, marginBottom: '16px' }}>{error}</div>}

                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: '14px', marginBottom: '14px' }}>
                        <div style={S.field}><label style={S.lbl}>No. Tagihan *</label><input className="fi" value={form.nomor_tagihan} onChange={e => setForm({ ...form, nomor_tagihan: e.target.value })} /></div>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px', marginBottom: '14px' }}>
                        <div style={S.field}>
                            <label style={S.lbl}>Pemasok *</label>
                            <select className="fi" value={form.pemasok} onChange={e => setForm({ ...form, pemasok: e.target.value })}>
                                <option value="">-- Pilih Pemasok --</option>
                                {pemasokList.map(p => <option key={p.id} value={p.id}>{p.nama}</option>)}
                            </select>
                        </div>
                        <div style={S.field}><label style={S.lbl}>No. Ref. Pemasok</label><input className="fi" placeholder="No. invoice dari pemasok" value={form.nomor_ref_pemasok} onChange={e => setForm({ ...form, nomor_ref_pemasok: e.target.value })} /></div>
                    </div>
                    <div style={{ ...S.field, marginBottom: '20px' }}><label style={S.lbl}>Keterangan</label><input className="fi" placeholder="Catatan..." value={form.keterangan} onChange={e => setForm({ ...form, keterangan: e.target.value })} /></div>
                    ...
                    <div style={{ border: '1px solid #e2e8f0', borderRadius: '10px', overflow: 'hidden', marginBottom: '20px' }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '12px 14px', background: '#f8fafc', borderBottom: '1px solid #e2e8f0' }}>
                            <span style={{ fontSize: '14px', fontWeight: '700', color: '#1a2e1a' }}>Item Tagihan</span>
                        ...
                        <table className="it">
                            ...
                            <tbody>
                                {form.items.map((item, i) => (
                                    <tr key={i}>
                                        <td><input className="fi" placeholder="Nama barang/jasa..." value={item.deskripsi} onChange={e => handleItemChange(i, 'deskripsi', e.target.value)} /></td>
                    ...
                    <div style={{ display: 'flex', gap: '12px', justifyContent: 'flex-end', paddingTop: '16px', borderTop: '1px solid #f1f5f9' }}>
                        ...
                        <button className="btn-primary" onClick={handleSave} disabled={saving}>{saving ? 'Menyimpan...' : 'Simpan Tagihan'}</button>
                    </div>
                </div>
            )}
            ...
            {view === 'detail' && selected && (
                <div style={S.card}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
                        <div>
                            <h2 style={{ fontSize: '18px', fontWeight: '700', color: '#1a2e1a', fontFamily: 'monospace' }}>{selected.nomor_tagihan}</h2>
                            <p style={{ fontSize: '14px', color: '#64748b', marginTop: '4px' }}>
                                {selected.pemasok_detail?.nama} · {selected.pemasok_detail?.tipe_label}
                                {selected.nomor_ref_pemasok && <span style={{ marginLeft: '10px', color: '#94a3b8' }}>Ref: {selected.nomor_ref_pemasok}</span>}
                            </p>
                        </div>
                        <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap', justifyContent: 'flex-end' }}>
                            ...
                            {selected.status === 'draft' && <button className="btn-gold" onClick={handleTerima}>Terima</button>}
                            {['diterima', 'sebagian'].includes(selected.status) && (
                                <button className="btn-primary" onClick={() => { setBayarForm({ tanggal: new Date().toISOString().split('T')[0], jumlah: selected.sisa_tagihan, metode: 'transfer', keterangan: '', akun: '' }); setBayarModal(true); setError(''); }}>
                                    Catat Pembayaran
                                </button>
                            )}
                    ...
                    <div style={{ marginBottom: '24px' }}>
                        <div style={{ fontSize: '13px', fontWeight: '700', color: '#64748b', textTransform: 'uppercase', marginBottom: '10px' }}>Item Tagihan</div>
                        <table className="dt">
                                ...
                                <tbody>
                                    {selected.pembayaran.map((p, i) => (
                                        <tr key={i}>
                                            ...
                                            <td>{p.keterangan || '-'}</td>
                                            <td style={{ textAlign: 'right', fontFamily: 'monospace', fontWeight: '600', color: '#dc2626' }}>Rp {formatRp(p.jumlah)}</td>
                                        </tr>
                                    ))}
                                </tbody>
                            </table>
...
const S = {
    ...
    summaryCard: { background: '#fff', borderRadius: '12px', padding: '20px 24px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)', border: '1px solid #f1f5f9' },

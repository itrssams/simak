export const getInvoiceDisplayAmounts = (invoice = {}) => {
  ...
  const totalPiutang = Number(invoice?.total_piutang ?? invoice?.sisa_tagihan ?? invoice?.total_tagihan ?? 0);
  const displayTotal = Number.isFinite(totalPiutang) && totalPiutang > 0 ? totalPiutang : totalTagihan;
